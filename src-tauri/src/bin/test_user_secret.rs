...
fn main() {
    ...
    let totp_std = TOTP::new_unchecked(Algorithm::SHA1, 6, 1, 30, secret.to_bytes().unwrap(), None, "".to_string());
    ...
    let secret_bytes = base32::decode(base32::Alphabet::Rfc4648 { padding: false }, unpadded).unwrap();
    let totp_ours = TOTP::new_unchecked(Algorithm::SHA1, 6, 1, 30, secret_bytes, None, "".to_string());
